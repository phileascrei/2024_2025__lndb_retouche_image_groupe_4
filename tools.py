...
class ImageEditor(tk.Tk):
    ...
    def display_on_canvas(self):
        if self.display_image:
            # Créer une copie de l'image pour redimensionner
            resized_image = self.display_image.copy()
            resized_image.thumbnail((self.zoom_factor * 700, self.zoom_factor * 500), Image.LANCZOS)
    ...
    def auto_adjust_exposure(self):
        self.exposure_value = determine_target_exposure(self.original_image)
    ...
    def auto_adjust_contrast(self):
        self.contrast_value = determine_target_contrast(self.original_image)
    ...
    def auto_adjust_saturation(self):
        self.saturation_value = determine_target_saturation(self.original_image)
        self.saturation_slider.set(self.saturation_value)
        self.apply_adjustments()

    def adjust_highlights(self, value):
        self.highlights_value = float(value) / 128
        self.apply_adjustments()

    def auto_adjust_highlights(self):
        self.highlights_value = 0.8
        self.highlights_slider.set(self.highlights_value * 128)
        self.apply_adjustments()

    def adjust_shadows(self, value):
        self.shadows_value = float(value) / 128
        self.apply_adjustments()

    def auto_adjust_shadows(self):
        self.shadows_value = 1.2
        self.shadows_slider.set(self.shadows_value * 128)
        self.apply_adjustments()

    def apply_adjustments(self):
        if self.original_image:
            image = self.original_image
            image = adjust_exposure(image, self.exposure_value)
            image = adjust_contrast(image, self.contrast_value)
            image = adjust_saturation(image, self.saturation_value)
            image = adjust_highlights(image, self.highlights_value)
            image = adjust_shadows(image, self.shadows_value)
