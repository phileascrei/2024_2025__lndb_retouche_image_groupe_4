...
class ImageEditor(tk.Tk):
    ...
    def auto_adjust_saturation(self):
        if self.display_image:
            # Analyser la saturation actuelle de l'image
            current_saturation = analyse_saturation(self.display_image)
            
            # Déterminer la saturation cible
            target_saturation = 1.0  # Valeur cible pour une saturation moyenne
            
            # Calculer le facteur d'ajustement
            factor = target_saturation / current_saturation
            
            # Appliquer l'ajustement de saturation
